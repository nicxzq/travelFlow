...
import { archiveTripExecution, createTripChange } from './model.ts';
import {
...
function stateWith(...changes) {
  return { originalPlan: baseTrip, initialSnapshot: baseTrip, changes };
}
...
test('distance ranks a known nearby point within two kilometers', () => {
  const distance = distanceInKilometers({ lat: 37.2, lng: 112.1 }, { lat: 37.21, lng: 112.1 });

  assert.ok(distance > 1 && distance < 2);
});

test('recommendation prompt names its verification limits', () => {
  const prompt = buildRecommendationPrompt({
    placeName: '平遥古城',
    availableHours: 4,
    tripTitle: '山西亲子行',
    remainingEvents: ['双林寺', '乔家大院'],
  });
...
test('an archived review reads the frozen snapshot while a live review keeps moving', () => {
  const rename = change('update', 'e1', { patch: { title: '平遥古城' } });
  const base = stateWith(rename);
  const archived = archiveTripExecution(base, foldTripExecution(base));
  const afterArchive = { ...archived, changes: [...archived.changes, change('cancel', 'e2')] };

  const archivedReview = getExecutionReview(afterArchive);
  const liveReview = getExecutionReview({ ...afterArchive, archive: undefined });

  assert.equal(archivedReview.archived, true);
  assert.equal(archivedReview.changesAfterArchive, 1);
  assert.equal(archivedReview.rows.find((row) => row.eventId === 'e1').title, '平遥古城');
  assert.equal(archivedReview.rows.find((row) => row.eventId === 'e2').cancelled, false);
  assert.equal(liveReview.archived, false);
  assert.equal(liveReview.rows.find((row) => row.eventId === 'e2').cancelled, true);
});

test('an event missing from the actual trip is reported, not silently unchanged', () => {
  const revised = {
    ...baseTrip,
    planRevision: 2,
    days: [
      { ...baseTrip.days[0], events: [baseTrip.days[0].events[0], { id: 'added', dayId: 'day-1', tripId: 'trip-a', title: '新增点', category: 'spot' }] },
      baseTrip.days[1],
    ],
  };

  const review = getExecutionReview({
    originalPlan: { ...baseTrip, planRevision: 1 },
    initialSnapshot: revised,
    changes: [],
  });
  const row = review.rows.find((entry) => entry.eventId === 'e2');

  assert.equal(review.baselineStale, true);
  assert.equal(row.missing, true);
  assert.equal(row.changed, true);
  assert.deepEqual(review.addedEventIds, ['added']);
});
